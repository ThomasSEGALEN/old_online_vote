<x-app-layout>
    <x-slot name="header">
        <a href="{{ route('votes.show', [$session, $vote]) }}" class="inline-flex justify-center items-center mr-2">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    @endif
                    <form action="{{ route('votes.update', [$session, $vote]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="w-full flex flex-row justify-between">
                            <div class="flex flex-col w-1/2 -mx-3">
                                ...
                                <div class="w-full px-3 mb-3 md:mb-6">
                                    ...
                                    <textarea class="@error ('description') is-invalid @enderror appearance-none block w-full bg-gray-100 rounded py-3 px-4 md:mb-0" id="descriptionInput" type="text" name="description">{{ $vote->description }}</textarea>
                                    @error ('description')
                                    <span class="text-red-600">{{ $message }}</span>
                                    @enderror
                                </div>
                            ...
                            <div class="flex flex-col w-1/2 h-full -mx-3">
                                <div class="w-full px-3 mb-3 md:mb-6">
                                    ...
                                    <div class="@error ('type_id') is-invalid @enderror form-check flex flex-row">
                                        ...
                                        id="typeInput-{{ $type->id }}" type="radio" name="type_id" value="{{ $type->id }}" @if ($vote->type->id === $type->id) checked @endif>
