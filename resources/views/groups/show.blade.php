<x-app-layout>
    <x-slot name="header">
        <a href="{{ route('groups.index') }}" class="inline-flex justify-center items-center mr-2">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="relative p-6 bg-white border-b border-gray-200">
                    <div class="absolute top-4 right-4">
                        @can ('update', $group)
                        <a href="{{ route('groups.edit', $group) }}" class="inline-flex justify-center items-center p-2 text-base font-medium text-gray-500 bg-gray-50 rounded-lg hover:text-gray-900 hover:bg-gray-100">
                        ...
                        @endcan
                        @can ('delete', $group)
                            <form class="inline-flex justify-center items-center p-2 text-base font-medium text-gray-500 bg-gray-50 rounded-lg hover:text-gray-900 hover:bg-gray-100" id="form-{{ $group->id }}" action="{{ route('groups.destroy', $group) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('DELETE')
                    ...
                    <div>
                        <div class="flex flex-col">
                            <span class="mb-4">{{ $group->name }}</span>
