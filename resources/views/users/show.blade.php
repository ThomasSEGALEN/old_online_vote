<x-app-layout>
    <x-slot name="header">
        <a href="{{ route('users.index') }}" class="inline-flex justify-center items-center mr-2">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="relative p-6 bg-white border-b border-gray-200">
                    <div class="absolute top-4 right-4">
                        @can ('update', $user)
                        <a href="{{ route('users.edit', $user) }}" class="inline-flex justify-center items-center p-2 text-base font-medium text-gray-500 bg-gray-50 rounded-lg hover:text-gray-900 hover:bg-gray-100">
                        ...
                        @endcan
                        @can ('delete', $user)
                            <form class="inline-flex justify-center items-center p-2 text-base font-medium text-gray-500 bg-gray-50 rounded-lg hover:text-gray-900 hover:bg-gray-100" id="form-{{ $user->id }}" action="{{ route('users.destroy', $user) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('DELETE')
